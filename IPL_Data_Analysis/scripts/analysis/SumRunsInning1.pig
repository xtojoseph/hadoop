...
FILTER_table = FILTER join_table  BY ball::over<=6 and ball::inning=='1';
Group_Table = GROUP FILTER_table BY match::id;
Sum_Table_inning1 = FOREACH Group_Table  GENERATE group as matchid,SUM(FILTER_table .total_runs) as sumruns;
STORE Sum_Table_inning1 INTO 'metastore.SumRunsInning1' USING org.apache.hive.hcatalog.pig.HCatStorer();