...
A = FOREACH A GENERATE 'sample.txt' AS filename, FLATTEN(TOKENIZE(string)) AS word;
B = LOAD '/home/hdoop/codebase/hadoop/PIG/InvertedIndex/sample2.txt' AS (string:chararray);
B = FOREACH B GENERATE 'sample2.txt' AS filename, FLATTEN(TOKENIZE(string)) AS word;
allWords = UNION A,B;
